...
import PlanLockedOverlay from '@/components/ui/PlanLockedOverlay';
...
export default function NutritionScreen() {
  ...
  // Fetch plan nutrition targets when active
  useEffect(() => {
    if (planStatus === 'ACTIVE') {
      setIsLoadingTargets(true);
      planService.getPlanNutritionTargets()
        .then((targets) => {
          setPlanTargets(targets);
          // Update nutrition data with plan targets
          if (targets) {
            setNutritionData(prev => ({
              ...prev,
              dailyTarget: {
                calories: targets.dailyCalories || prev.dailyTarget.calories,
                protein: targets.macros?.protein || prev.dailyTarget.protein,
                carbs: targets.macros?.carbs || prev.dailyTarget.carbs,
                fats: targets.macros?.fats || prev.dailyTarget.fats,
              }
            }));
          }
        ...
        .catch((error) => {
          console.error('[NutritionScreen] Error fetching nutrition targets:', error);
          setIsLoadingTargets(false);
        });
    } else {
      setPlanTargets(null);
    ...
    <SafeAreaView style={styles.safeArea}>
      {isLocked && (
        <PlanLockedOverlay
          ...
          message="Complete your AI plan to unlock nutrition tracking"
      ...
      >
        <View style={[styles.responsiveContainer, { width: contentWidth }]}>
          ...
          {/* Calorie Ring */}
          <View style={[styles.calorieRingContainer, isLocked && styles.locked]}>
            <CalorieRing
